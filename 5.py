...
def save_data():
    ...
    try:
        # Changed to %Y for 4-digit year format for robustness
...
def export_to_excel():
    """Exports all appointment data to an Excel file, allowing user to choose save location."""
    try:
        # Read CSV, explicitly specifying dtype for 'Phone'
        df = pd.read_csv(FILE_NAME, dtype={'Phone': str})
...
# Corrected date_pattern to "%m/%d/%Y" for 4-digit year format
calendar = Calendar(left_panel, selectmode="day", date_pattern="mm/dd/%Y",
