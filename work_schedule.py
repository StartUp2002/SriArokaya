...
def load_upcoming():
    ...
    try:
        ...
        now = datetime.now()
        df = df[df["EndDatetime"] >= now]  # เหลือเฉพาะที่กำลังทำอยู่หรือล่วงหน้า
...
# ---------------- GUI ----------------
root = tk.Tk()
root.title("Home | Thai Traditional Massage Queue System")
...
tk.Label(root, text="\U0001F5D5 Select Date").pack()
...
tk.Button(root, text="\U0001F3E0 Go to Home", command=go_home).pack(pady=5)
...
tk.Label(root, text="\U0001F5D6 Upcoming Appointments").pack()
