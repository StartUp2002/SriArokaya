...
def save_data():
    ...
    try:
        ...
        # If that fails, try parsing with 2-digit year format
        try:
            date_obj = datetime.strptime(selected_date_str, "%m/%d/%y")
            date_formatted = date_obj.strftime("%Y-%m-%d")
    ...
    try:
        ...
        if selected_date:
            ...
            if hasattr(root, '_gantt_canvas') and root._gantt_canvas:
                root._gantt_canvas.get_tk_widget().destroy() # Destroy the canvas widget
                root._gantt_canvas = None # Clear the reference
...
def draw_gantt_chart(df_data, selected_date):
    ...
    for widget in chart_frame.winfo_children():
        widget.destroy()
    # Also ensure any old canvas is destroyed
    if hasattr(root, '_gantt_canvas') and root._gantt_canvas:
        root._gantt_canvas.get_tk_widget().destroy()
...
def export_to_excel():
    """
    Exports all appointment data to an Excel file, allowing the user to choose the save location.
    Data is organized into sheets by year, and within each sheet, appointments are grouped by month.
    """
    try:
        # Read CSV, explicitly specifying dtype for 'Phone'
        df = pd.read_csv(FILE_NAME, dtype={'Phone': str})
        ...
        df['Month'] = df['Date'].dt.strftime('%B') # Full month name, e.g., 'July'

        # Open a file dialog to ask for the save location
        file_path = filedialog.asksaveasfilename(
            defaultextension=".xlsx",
            filetypes=[("Excel files", "*.xlsx"), ("All files", "*.*")],
            title="Save Appointment Schedule as"
        )
        ...
        if file_path:  # If the user didn't cancel the dialog
            with pd.ExcelWriter(file_path, engine='xlsxwriter') as writer:
                ...
                for year in years:
                    ...
                    df_year = df_year.sort_values(by=['Date', 'StartTime'])

                    # Prepare the data for the sheet (excluding the temporary 'Year' column)
                    # We add 'Month' column at the beginning for better organization within the sheet
                    columns_to_export = ["Date", "Name", "StartTime", "EndTime", "Phone", "Note"]
                    df_year_for_export = df_year[['Month'] + columns_to_export]
                    
                    # Write the year's data to a new sheet named after the year
                    sheet_name = str(year)
                    df_year_for_export.to_excel(writer, sheet_name=sheet_name, index=False)

                    # Optional: Add some formatting or notes to the sheet
                    workbook = writer.book
                    worksheet = writer.sheets[sheet_name]
                    
                    # Auto-fit columns
                    for i, col in enumerate(df_year_for_export.columns):
                        max_len = max(df_year_for_export[col].astype(str).map(len).max(), len(col)) + 2
        ...
        messagebox.showerror("Export Failed", "Module 'openpyxl' or 'xlsxwriter' not found. Please install them: pip install openpyxl xlsxwriter")
    except Exception as e:
...
def on_calendar_select(event=None):
    ...
    try:
        ...
        selected_date_str = calendar.get_date()
        try:
            # Try parsing with 4-digit year format first (e.g., 07/04/2025)
            selected_date = datetime.strptime(selected_date_str, "%m/%d/%Y").strftime("%Y-%m-%d")
        except ValueError:
            # If that fails, try parsing with 2-digit year format (e.g., 07/04/25)
            selected_date = datetime.strptime(selected_date_str, "%m/%d/%y").strftime("%Y-%m-%d")
...
style.configure('TButton', background=ACCENT_COLOR, foreground='white', font=BUTTON_FONT, padding=8, relief='flat')
style.map('TButton', background=[('active', PRIMARY_COLOR)], foreground=[('active', 'white')])

style.configure('Treeview.Heading', font=SUBHEADER_FONT, background=PRIMARY_COLOR, foreground='white', padding=5)
style.configure('Treeview', font=NORMAL_FONT, rowheight=26, background='white', foreground=TEXT_COLOR, fieldbackground='white')
...
tk.Label(left_panel, text="🗓️ Schedule New Appointment", font=SUBHEADER_FONT, fg=PRIMARY_COLOR, bg=BACKGROUND_COLOR).pack(pady=(8, 15))
...
phone_entry = ttk.Entry(left_panel, font=NORMAL_FONT, width=30)
...
# Ensure calendar date_pattern outputs 4-digit year to match common datetime formats
calendar = Calendar(left_panel, selectmode="day", date_pattern="mm/dd/yyyy", 
...
start_hour_spinbox.set(9)
start_hour_spinbox.pack(side=tk.LEFT, padx=(0, 5))
...
start_minute_spinbox.set(0)
start_minute_spinbox.pack(side=tk.LEFT, padx=(5, 0))
...
end_time_frame.pack(pady=(0, 15), padx=5, fill='x', expand=False)
...
end_minute_spinbox.set(0)
end_minute_spinbox.pack(side=tk.LEFT, padx=(5, 0))

# Add Note Text Area
tk.Label(left_panel, text="Note:", bg=BACKGROUND_COLOR, fg=TEXT_COLOR, font=NORMAL_FONT).pack(anchor='w', padx=5, pady=(4, 0))
note_text = tk.Text(left_panel, font=NORMAL_FONT, width=30, height=5, wrap="word", bd=2, relief="groove")
...
ttk.Button(button_frame, text="➕ Save Appointment", command=save_data, style='TButton').pack(side=tk.LEFT, padx=5, pady=5, expand=True, fill='x')
ttk.Button(button_frame, text="📊 Export to Excel", command=export_to_excel, style='TButton').pack(side=tk.LEFT, padx=5, pady=5, expand=True, fill='x')
...
right_panel.grid_rowconfigure(2, weight=1) # For upcoming_tree_frame
right_panel.grid_rowconfigure(5, weight=1) # For tree_frame
right_panel.grid_rowconfigure(7, weight=2) # For chart_frame
right_panel.grid_columnconfigure(0, weight=1)

# Upcoming Appointments Section
tk.Label(right_panel, text="⏳ Upcoming Appointments", font=SUBHEADER_FONT, fg=PRIMARY_COLOR, bg=BACKGROUND_COLOR).grid(row=0, column=0, pady=(8, 10), sticky='ew')

# Filter controls for Upcoming Appointments
upcoming_filter_frame = tk.Frame(right_panel, bg=BACKGROUND_COLOR)
upcoming_filter_frame.grid(row=1, column=0, sticky='ew', padx=4, pady=(0, 8))

tk.Label(upcoming_filter_frame, text="Filter by Name:", bg=BACKGROUND_COLOR, fg=TEXT_COLOR, font=NORMAL_FONT).pack(side=tk.LEFT, padx=(0, 8))
upcoming_filter_entry = ttk.Entry(upcoming_filter_frame, font=NORMAL_FONT, width=25)
upcoming_filter_entry.pack(side=tk.LEFT, expand=True, fill='x', padx=(0, 8))
...
upcoming_tree_frame.grid(row=2, column=0, pady=(0, 15), padx=4, sticky='nsew')
...
upcoming_tree.column("Date", anchor="center", width=90)
upcoming_tree.column("Name", anchor="w", width=120)
upcoming_tree.column("StartTime", anchor="center", width=70)
upcoming_tree.column("EndTime", anchor="center", width=70)
upcoming_tree.column("Phone", anchor="center", width=100)
upcoming_tree.column("Note", anchor="w", width=180)
...
tk.Label(right_panel, text="📅 Appointments for Selected Date", font=SUBHEADER_FONT, fg=PRIMARY_COLOR, bg=BACKGROUND_COLOR).grid(row=3, column=0, pady=(8, 10), sticky='ew')

filter_frame = tk.Frame(right_panel, bg=BACKGROUND_COLOR)
filter_frame.grid(row=4, column=0, sticky='ew', padx=4, pady=(0, 8))

tk.Label(filter_frame, text="Filter by Name:", bg=BACKGROUND_COLOR, fg=TEXT_COLOR, font=NORMAL_FONT).pack(side=tk.LEFT, padx=(0, 8))
filter_entry = ttk.Entry(filter_frame, font=NORMAL_FONT, width=25)
filter_entry.pack(side=tk.LEFT, expand=True, fill='x', padx=(0, 8))
...
tree_frame.grid(row=5, column=0, pady=(0, 15), padx=4, sticky='nsew')
...
tree.column("Date", anchor="center", width=90)
tree.column("StartTime", anchor="center", width=70)
tree.column("EndTime", anchor="center", width=70)
tree.column("Phone", anchor="center", width=100)
tree.column("Note", anchor="w", width=180)
tree.pack(fill='both', expand=True)
tree_scroll.config(command=tree.yview)

# Gantt Chart Frame
tk.Label(right_panel, text="📊 Daily Appointment Gantt Chart", font=SUBHEADER_FONT, fg=PRIMARY_COLOR, bg=BACKGROUND_COLOR).grid(row=6, column=0, pady=(8, 10), sticky='ew')
chart_frame = tk.Frame(right_panel, bg=BACKGROUND_COLOR, bd=2, relief="solid")
chart_frame.grid(row=7, column=0, pady=(0, 8), padx=4, sticky='nsew')
