...
def load_data(selected_date=None, filter_name=""):
    ...
    current_selected_date = selected_date # Update global selected date
    ...
    try:
        ...
        if selected_date:
            draw_gantt_chart(df_filtered_by_date, selected_date) # Chart always shows all for selected date
        else:
...
def on_calendar_select(event=None):
    ...
    try:
        ...
        current_selected_date = selected_date # Update global
...
def go_home():
    ...
    global current_selected_date
    current_selected_date = None # Reset selected date
    filter_entry.delete(0, tk.END) # Clear filter entry
    root.title(f"Home | {APP_TITLE}")
    load_data(selected_date=None) # Load all data
    calendar.selection_clear() # Clear calendar selection
...
upcoming_frame.grid(row=1, column=0, pady=(0, 10), padx=4, sticky='nsew')
...
# Filter controls for "Appointments for Selected Date"
filter_frame = tk.Frame(right_panel, bg=BACKGROUND_COLOR)
filter_frame.grid(row=3, column=0, sticky='ew', padx=4, pady=(0, 4)) # Adjusted row and padding

tk.Label(filter_frame, text="Filter by Name:", bg=BACKGROUND_COLOR, fg=TEXT_COLOR).pack(side=tk.LEFT, padx=(0, 5))
filter_entry = ttk.Entry(filter_frame, font=NORMAL_FONT, width=20)
filter_entry.pack(side=tk.LEFT, expand=True, fill='x', padx=(0, 5))
filter_entry.bind("<KeyRelease>", apply_filter) # Real-time filtering
...
tree_frame.grid(row=4, column=0, pady=(0, 10), padx=4, sticky='nsew') # Adjusted row to accommodate filter controls
...
# Gantt Chart Frame
tk.Label(right_panel, text="📊 Daily Appointment Gantt Chart", font=SUBHEADER_FONT, fg=PRIMARY_COLOR, bg=BACKGROUND_COLOR).grid(row=5, column=0, pady=(8, 8), sticky='ew') # Adjusted row
chart_frame = tk.Frame(right_panel, bg=BACKGROUND_COLOR, bd=2, relief="solid")
chart_frame.grid(row=6, column=0, pady=(0, 4), padx=4, sticky='nsew') # Adjusted row
...
# =============================================================================
load_upcoming()
load_data(selected_date=None) # Load all data initially
